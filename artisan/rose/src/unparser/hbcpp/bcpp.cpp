...
************************************************************************************ */
    ...
    pInputFile = fmemopen((char *) src.c_str(), src.size(), "r");
    
    char *buffer = new char[src.size() *10 + 1];
    memset(buffer, 0, src.size()*10);
    pOutputFile = fmemopen(buffer, src.size() *10, "wb"); 

    // #### Lets do some code crunching !
    ProcessFile (pInputFile, pOutputFile, settings);
    fflush(pInputFile); fflush(pOutputFile);
    fclose(pInputFile); fclose(pOutputFile);
